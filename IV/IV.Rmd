...
Instead of trying to strip away all the undesirable variation using controls, an instrumental variable (IV) is a source of variation that allows to isolate just the interested front-door path. IV designs attempt to mimic a randomized experiment, but using statistics and opportune settings instead of actually being able to influence or randomize anything.
...
An IV design works in the exact same way:
...
**Relevance of the instrument**
<br>
- The key idea of IV is to use that part of treatment $X$ that is explained by the instrument $Z$ is used to explain the outcome $Y$. In its basic form, IV gives us $\frac{\text{Cov}(Z,\,Y)}{\text{Cov}(Z,\,X)}$. 
...
- In many court systems, the process of assigning a judge to your trial is more or less random. This is important because some judges are harsher than and others.
...
- To use GMM to estimate the expected value of a variable $Y$, it can be said that the difference between the estimator $\mu$ and the population expected value $E(Y)$ should be zero 
...
- Different ways of IV estimation can produce different weighted average treatment effects

- LATE is estimated in most cases, although the specifics on *which* LATE we are getting change from estimator to estimator

- There is a common terminology for these weights, in particular the 0s. The sample can be devided into three groups.
...
**Q: Why is sequential testing of instrument validity using t-tests *invalid?* **
...
- Run the second-stage model but include the instrument as a control, i.e., to estimate
...
- Remamber that IV regression uses the parts of the treatment/endogenous variable $X$ and the outcome $Y$ that are predicted by the instrument $Z$
...
- If $\kappa = 1$, then the prediction is scaled by 1, and we ending up with 2SLS
...
- **Fuller’s  $\alpha$ adjustment** can be made to $\kappa$ as $$\kappa = \hat{\kappa}-\frac{\alpha}{(N-N_1)},$$ where $N$ is the number of observation and $N_1$ is the number of instruments
...
- The first tries to do what it can with a mostly-valid instrument:
  ...
  As long as the instrument impacts treatment, we can get useful inferences. This may require a very differentiated analysis. The inferences we get might not be quite as precise as when you do have a 'fully valid' IV.
