---
title: "<a href='https://github.com/mca91/kausalanalyse_slides/raw/SoSe26/IV/IV.pdf'>Instrumental variables</a>"
...
```{r, echo=F, cache=F, out.extra = 'style="display:block; margin-right:auto; margin-left:auto; margin-top:50px;"', fig.height=3, out.width='65%', dpi=190, fig.cap=paste0(': Causal Diagram'), fig.align='center',include=FALSE}
coord_dag <- list(
  x = c(a = 1, b = 3, c = 5, d = 7 ),
  y = c(a = 2, b = 1, c = 2, d = 1)
)
...
<br>
```{r, echo=F, cache=F, fig.cap=paste0(fig_counter(), ': Causal Diagram')}
m <- DiagrammeR::grViz("dot_files/Treatment_Outcome_pg3.dot", height = 250, width = "100%")
...
# Assumptions for IV
<br>
```{r, echo=F, cache=F, out.extra = 'style="display:block; margin-right:auto; margin-left:auto;"', fig.height=4, out.width='65%', dpi=190, fig.cap=paste0(fig_counter(), ': A causal diagram amenable to IV estimation'), fig.align='center',include=FALSE}
...
```{r, echo=F, cache=F, fig.cap=paste0(fig_counter(), ': A causal diagram amenable to IV estimation')}
m <- DiagrammeR::grViz("dot_files/Treatment_Outcome_pg8.dot", height = 400, width = "100%")
widgetframe::frameableWidget(m)
```
---
#Assumptions for IV
...
- Bad small-sample performance. For real data, the relationship between $Z$ and the $\text{'non-}X \text{'}$ part of $Y$ is going to be somewhat nonzero, just by random chance 
...
.blockquote.font90[
### Example: GMM estimator for the upper bound of a uniform RV

- (G)MM expresses population moments (e.g., the expected value) as functions of the parameter(s). Those expressions are then set equal to the sample moments and solved for the parameters.
- E.g., suppose we have  $Y_i \sim U[0, a]$ such that $E(Y) = \frac{a}{2}$
- Plugging in the sample average for $E(Y)$ results in $\displaystyle \frac{1}{N} \sum_{i = 1}^{N} Y_i = \dfrac{\hat{a}}{2}$
- Rearranging yields $\displaystyle \hat{a} = 2 \cdot \left( \frac{1}{N} \sum_{i = 1}^{N} Y_i \right)$
]
...
- When the number of instruments is bigger than the number of treatment/endogenous variables, we say that the model is **overidentified**
  ...
  **When the model is overidentified, 2SLS and GMM typically differ!**
...
---
# IV estimation and treatment effects: LATE
<br>
...
- Li, on the other hand, does not respond to the ads at all &mdash. So it turns out he makes no difference to the 2SLS estimate. He gets a weight of $0$.
]]
...
- Hence, different IVs will tend to produce different results
...
- It is sometimes argued a nonzero coefficient on the instrument Z suggests a violation of the validity assumption
...
There is a handy and easy-to-implement method for incorporating a binary treatment variable into 2SLS popularized by Wooldridge (2010)
