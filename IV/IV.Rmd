...
.blockquote[
### Example: The sample mean is a GMM estimator of the expected value $\mu = E(Y)$

- The GMM estimator, demands that the difference between the estimator of $\mu$ and the population expected value $E(Y)$ should be zero 

  &rarr; $\mu - E(Y) = 0$ is a moment condition for the estimation.
  
- E. g., suppose we have  $Y_i \sim [0, a]$
- Also $E(Y) = \dfrac{a + b}{2} = \dfrac{a}{2}$
- Plugging in the (G)MM sample estimator $\bar(y)$ for $E(Y)$ results in $\bar(y)= \dfrac{1}{N} \sum_{i = 1}^{N} Y_i = \dfrac{a}{2}$
- Rearranging yields $a = 2 \cdot \bar(y)$
