---
title: "<a href='https://github.com/mca91/kausalanalyse_slides/raw/SoSe26/Simulation/Simulation.pdf'>Simulation</a>"
...
A typical statistical simulation only has a few parts:
...
2. Apply an estimator to the sample of 100 coin flips. E.g. calculate the proportion of heads in that sample.
...
In the context of causal inference, simulation can determine if an estimator provides, *on average*, the true causal effect. 
...
- Different choices can be made in matching. The most important choice is between **reducing bias** and **improving precision.**
...
- It might be easier if the bias reduction is not very high, but the precision savings are huge. **Horse race simulation** is useful for this.
...
- If we want a linear regression coefficient estimator on $X$ to be unbiased  for the causal effect of $X$, we need to assume that $X$ is unrelated to the error term (no open back doors).
...
- If the analyst has the impression that an effect is probably not that central or important or has a small effect on a part of the system where a lot of other “stuff” is going on, a power analysis should be performed to see a necessary sample size. 

- If the variation of an effect over groups is of interest, then a power analysis is a good idea. Finding differences between groups in an effect takes more data to find than the effect itself.
...
- There is a trade-off: the more careful one is about false positives, the more likely one will get a false negative.
...
- In order to do power analysis, one must fill in the values for four of the five pieces, so that power analysis can determine the fifth one
...
2. Randomly sample $N$ observations from that data set *with replacement*, i.e. allowing the same observation to be drawn more than once. This is called a **bootstrap sample.**
...
- The data should be reasonably well behaved. For example, if the variables follow extremely highly skewed distributions, bootstrap will not work well. 
...
---
## The bootstrap
#### Bootstrap estimator for $\mu$ and $95\%$ bootstrap CI

<div class="thecontainer">
  <div id="observablehq-svg-cfafeb80"></div>
  <div class="right-aligned">
    <ul style="list-style-type:none;">
      <li><div id="observablehq-viewof-B-cfafeb80"></div></li>
      <li><div id="observablehq-viewof-BW-cfafeb80"></div></li>
      <li><div id="observablehq-viewof-speed-cfafeb80"></div></li>
      <li><div id="observablehq-viewof-n-cfafeb80"></div></li>
    </ul>
  </div>
</div>
...
import define from "https://api.observablehq.com/d/62d24833f3dbd27a@783.js?v=4";
new Runtime().module(define, name => {
  if (name === "svg") return new Inspector(document.querySelector("#observablehq-svg-cfafeb80"));
  if (name === "viewof B") return new Inspector(document.querySelector("#observablehq-viewof-B-cfafeb80"));
  if (name === "viewof BW") return new Inspector(document.querySelector("#observablehq-viewof-BW-cfafeb80"));
  if (name === "viewof speed") return new Inspector(document.querySelector("#observablehq-viewof-speed-cfafeb80"));
  if (name === "viewof n") return new Inspector(document.querySelector("#observablehq-viewof-n-cfafeb80"));
  return ["dataset","mX","stdX","newXDomain"].includes(name);
});
</script>
...
- A wild bootstrap follows the general outline of residual resampling, except that residuals are not resampled. Each residual must line up with its original observation. Instead of resampling, the residuals get transformed by a function.

<!--
- For a wild bootstrap, the general outline of residual resampling is followed. Except that suitably transformed residuals are not resampled. Each residual needs to be lined up with its original observation each time the bootstrap data is created.
-->