...
**The cluster/block bootstrap**
 ...
 - When applying a bootstrap to time series data, the time series is first divided into blocks of continuous time. The blocks themselves are often determined by one of several different available an optimal-block-length algorithms
...
2. Use the analysis to get a predicted outcome value $\widehat{Y}$ and also the residual $\hat{r}$ which is the difference between the actual outcome and the predicted outcome, $$r=Y-\widehat{Y}.$$
...
The idea of the algorithm is that the predictors in the model never change with this process. Hence, the residual bootstrap mimmicks whatever kind of interdependence they have. 

<br>
**Caution:**

- A downsie of residual resampling is that it does not perform well if the errors are in any way related to the predictors.
