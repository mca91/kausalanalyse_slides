...
**Motivation**
<br><br>
Fixed effects are often remedy for two problems:
<br>
...
```{r, echo=F, cache=F, out.extra = 'style="display:block; margin-right:auto; margin-left:auto;"', fig.height=4, out.width='55%', dpi=190, fig.cap=paste0(fig_counter(), ': Causes of trade with Germany'), fig.align='center'}
coord_dag <- list(
...
```{r, echo=F, cache=F, out.extra = 'style="display:block; margin-right:auto; margin-left:auto; margin-top:30px"', fig.height=3, out.width='55%', dpi=190, fig.cap=paste0(fig_counter(), ': Simplified causes of trade with Germany'), fig.align='center'}
coord_dag <- list(
  x = c(a = 0, b = 2, c = 0, d = 2),
  y = c(a = 1, b = 1, c = 0, d = 0)
)
...
```{r, echo=F, cache=F, out.extra = 'style="display:block; margin-right:auto; margin-left:auto;"', fig.height=4, out.width='65%', dpi=190, fig.cap=paste0(fig_counter(), ': Healthy eating reminders and healthy eating for four individuals'), fig.align='center'}
set.seed(2000)
...
```{r, echo=F, cache=F, out.extra = 'style="display:block; margin-right:auto; margin-left:auto;"', fig.height=4, out.width='65%', dpi=190, fig.cap=paste0(fig_counter(), ': Healthy eating reminders and healthy eating for four individuals with individual means'), fig.align='center'}
ggplot(tb, aes(x = IntensityOfReminders,
               y = HealthyEatingScore, 
               shape = Individual)) + 
  geom_point() + 
...
```{r, echo=F, cache=F, out.extra = 'style="display:block; margin-right:auto; margin-left:auto;"', fig.height=4, out.width='60%', dpi=190, fig.cap=paste0(fig_counter(), ': Healthy eating reminders and healthy eating for four individuals with individual means'), fig.align='center'}
...
- That individual without an own intercept is still in the analysis, but it does not get it's own coefficient: estimates are interpreted *relative* to that individual.
...
- Sometimes researchers will use the fixed effects estimates to make claims about the individuals being evaluated 
...
- Data is on how much each person earns per year $Y_{it}$ and on how many hours of training they received that year $X_{it}$. Earnings in 2009 were $10,000 below what they normally are (due to recession)

- Anthony, earns $120,000 per year, a lot more money than the average person ($40,000). In 2009 Anthony only earned $116,000 which is above average, but that can be explained by the fact that Anthony earns a lot.
...
- Random effects assumes that the $\beta_is$ come from a known random distribution, for example assuming that the $\beta_is$ follow a normal distribution
...
- One common way around this problem is to use the **Durbin-Wu-Hausman test*
...
- An assumption of the regression model is that the errors are independent. Then parts of the DGP for the outcome variable that are not in the model are effectively random.
   ...
   This assumption is *necessary to obtain correct regression standard errors* !
...
- Either the fixed effect groups/individuals in the data need to be a non-random sample of the population, or, within fixed effect groups/individuals, the treatment variable is assigned in a clustered way.
...
- That is fine in a linear model, but in a nonlinear model we might run into the **incidental parameters problem**, i.e. we can not handle well estimating that many groups
  
  &rarr; The estimates start getting noisy!
