---
title: "<a href='https://github.com/mca91/kausalanalyse_slides/raw/SoSe26/FixedEffects/FixedEffects.pdf'>Fixed Effects</a>"
...
```{r, echo=F, cache=F, out.extra = 'style="display:block; margin-right:auto; margin-left:auto;"', fig.height=4, out.width='55%', dpi=290, fig.cap=paste0(fig_counter(), ': Causes of trade with Germany'), fig.align='center',include=FALSE}
coord_dag <- list(
...
```{r, echo=F, cache=F, fig.cap=paste0(fig_counter(), ': Causes of trade with Germany')}
m <- DiagrammeR::grViz("dot_files/TradeWithGermany.dot", height = 300, width = "100%")
...
The effect can be identified *without* controlling for each of the variables &mdash; controlling only for country would be enough.

```{r, echo=F, cache=F, out.extra = 'style="display:block; margin-right:auto; margin-left:auto; margin-top:30px"', fig.height=3, out.width='55%', dpi=290, fig.cap=paste0(fig_counter(), ': Simplified causes of trade with Germany'), fig.align='center',include=FALSE}
coord_dag <- list(
  x = c(a = 0, b = 2, c = 0, d = 2),
  y = c(a = 1, b = 1, c = 0, d = 0)
)
...
```{r, echo=F, cache=F, fig.cap=paste0(fig_counter(), ': Simplified causes of trade with Germany')}
m <- DiagrammeR::grViz("dot_files/TradeWithGermany_pg6.dot", height = 300, width = "100%")
widgetframe::frameableWidget(m)
```
...
- All four crosses are slid on top of each other
...
With the different individuals moved on top of each other, a clearer picture emerges, with a positive relationship between the intensity of reminders and the scores. 
...
ggplot(tb, aes(x = Intensity.R,
  ...
  theme(text         = element_text(size = 13),
        axis.title.x = element_text(size = 13),
        axis.ticks.y = element_blank()#,
#        legend.position = c(.7, .8) #, legend.background = element_rect(fill = 'white')
        )
...
**Method 1:** extracting the within variation (absorbing the fixed effects).
  1. Obtain $Y_{it} - \bar{Y_i}$, i.e., for each individual subtract the individual specific mean $\bar{Y_i}$ from the dependent variable $Y_{it}$
...
- The **Durbin-Wu-Hausman test** is useful in this context.

---
# Fixed effects vs. random effects
#### A comparison of panel estimators

<div class="thecontainer">
  <div id="observablehq-theplot-feda76a0"></div>
  <div class="right-aligned">
    <ul style="list-style-type:none;">
      <li><div id="observablehq-viewof-showgroups-feda76a0"></div></li>
      <li><div id="observablehq-viewof-showdgp-feda76a0"></div></li>
      <li><div id="observablehq-viewof-regtype-feda76a0"></div></li>
    </ul>
  </div>
</div>

<div id="observablehq-viewof-showgroups-feda76a0"></div>
<div id="observablehq-viewof-showdgp-feda76a0"></div>
<div id="observablehq-viewof-regtype-feda76a0"></div>


<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/@observablehq/inspector@5/dist/inspector.css">
<script type="module">
import {Runtime, Inspector} from "https://cdn.jsdelivr.net/npm/@observablehq/runtime@5/dist/runtime.js";
import define from "https://api.observablehq.com/@mca91/panelcomp@252.js?v=4";
new Runtime().module(define, name => {
  if (name === "viewof showgroups") return new Inspector(document.querySelector("#observablehq-viewof-showgroups-feda76a0"));
  if (name === "viewof showdgp") return new Inspector(document.querySelector("#observablehq-viewof-showdgp-feda76a0"));
  if (name === "viewof regtype") return new Inspector(document.querySelector("#observablehq-viewof-regtype-feda76a0"));
  if (name === "theplot") return new Inspector(document.querySelector("#observablehq-theplot-feda76a0"));
});
</script>
...
- However, such an assumption is questionable when it comes to fixed effects:
...
<br>
- The other method for getting rid of between variation is subtracting out the within-group means. 
 - This no longer works because the outcome variable is not a continuous variable any more
 - The mean is no longer a good representation of _what needed to be subtracted out to get rid of the between variation._
...
$$Y_{it}=\beta_i+\beta_1X_{it}+\epsilon_{it}$$
  it can be said that $\beta_i$ has its own model equation,
...
- This allows us to deal with the correlation between the $\beta_i$ and the $X_{it}$ because it is *modeled directly*
