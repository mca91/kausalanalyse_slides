...
---
## Appendix: matching example with `R`

### Matching in `R ` with the`Matching::Match()` function
...
---
## Appendix: matching example with `R` &mdash; ctd.

### Constructing matching ourself
...
for (i in 1:length(X.s.T)){
  ...
  minimalDistance[i] <- min(dist[i,])
  if (minimalDistance[i] < 1){ # only match if inside caliper
    # match all that share closest distance (ties)
    matches.i <- which(dist[i,]==minimalDistance[i]) 
...
length(Y.T) # Original number of treated obs
# drop treated observations outside of caliper 1
Y.T <- Y.T[(minimalDistance < 1)] # treated observations that have a match
length(Y.T) # Matched number of observations
