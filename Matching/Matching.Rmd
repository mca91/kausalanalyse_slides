...
- The main idea is:
 ...
 - If a match is encountered, then that observation gets dropped.
...
- In order to ensure that no variable ends up being weighted more heavily just because it’s on a bigger scale, we take $S$ (which is the covariance matrix for all matching variables) and  then divide the values of all matching variables by it. $S$ provides standardization and takes covariates into account.  
