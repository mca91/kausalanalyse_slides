---
title: "<a href='https://github.com/mca91/kausalanalyse_slides/raw/SoSe26/Matching/Matching.pdf'>Matching</a>"
...
Matching is usually applied to binary treated/untreated treatment variables, where the control group is chosen such that it is very similar to the treatment group.
]]
...
- Now, the gender difference between the treated and untreated groups gets eliminated. 
...
- The weights may be constructed as: Give a weight of 1 to *everyone* who is treated, $80/500 = 0.16$ to all untreated men, and $20/500 = 0.04$ to all untreated women
...
```{r, echo=F, cache=F, out.extra= 'style="display:block; margin-right:auto; margin-left:auto;"', fig.height=3, out.width='55%', dpi=190, fig.cap=paste0(fig_counter(), ': Causal Diagram'), fig.align='center',include=FALSE}
coord_dag <- list(
  x = c(a = 2, b = 5, c = 8, d = 2,e=8 ),
  y = c(a = 2, b = 2, c = 2, d = 1,e=1 )
)
dag_object <- ggdag::dagify(d ~ a,
                            d ~ b,
                            d ~ c,
                            e ~ a,
                            e ~ b,
...
```{r, echo=F, cache=F, fig.cap=paste0(fig_counter(), ': Causal diagram: distance matching based on A, B, and C')}
m <- DiagrammeR::grViz("dot_files/CD_pg11.dot", height = 280, width = "100%")
widgetframe::frameableWidget(m)
```
---
# Distance matching

#### 1-to-k Euclidean distance matching (with replacement)

<div class="thecontainer">
  <div id="observablehq-styles-4f5a50ba"></div>
  <div id="observablehq-theplot-4f5a50ba"></div>
  <div class="right-aligned">
    <ul style="list-style-type:none;">
      <li><div id="observablehq-viewof-mtoggle-4f5a50ba"></div></li>
      <li><div id="observablehq-viewof-caliper-4f5a50ba"></div></li>
      <li><div id="observablehq-viewof-k-4f5a50ba"></div></li>
    </ul>
    <ul>
      <li>Use the checkbox to toggle between highlighting of</li>
        <ul>
          <li>the matched dataset (1-to-k matching of control to treatment units with replacement) and </li>
          <li>the (potential) matches of the currently selected observation</li>
        </ul>
      <li>Click an observation to view its (potential) matches</li>
      <li>Varying the slider inputs updates the selection</li>
  </ul>
  </div>
</div>
...
import define from "https://api.observablehq.com/d/62067a02a72c9f90.js?v=4";
new Runtime().module(define, name => {
  if (name === "theplot") return new Inspector(document.querySelector("#observablehq-theplot-4f5a50ba"));
  if (name === "viewof k") return new Inspector(document.querySelector("#observablehq-viewof-k-4f5a50ba"));
  if (name === "viewof caliper") return new Inspector(document.querySelector("#observablehq-viewof-caliper-4f5a50ba"));
  if (name === "viewof mtoggle") return new Inspector(document.querySelector("#observablehq-viewof-mtoggle-4f5a50ba"));
  if (name === "styles") return new Inspector(document.querySelector("#observablehq-styles-4f5a50ba"));
});
</script>
...
- A good choice of a single matching control observation for row number 10,305, would be row 27,281, which was not late in April (and so was untreated), and has a April bill of NT$ 91,609, (distance $|91630-91609|=21$)
...
---
## Matching example with `R`

#### Matching with the `Match()` function

.font90[
```{r, echo = TRUE, eval=-1}
library(Matching) # Package for matching
br <- causaldata::black_politicians

Y <- br %>% pull(responded) # Outcome
D <- br %>% pull(leg_black) # Treatment
X <- br %>% dplyr::select(blackpercent) %>% as.matrix() # Matching Varibale
  

M <- Match(Y, D, X, Weight = 2, # Weight = 2, denotes Mahalanobis distance 
           caliper = 1, distance.tolerance = 1e-20)
summary(M)
```
]

---
## Matching example with `R` &mdash; ctd.

#### Constructing matching ourselves

.font90[
```{r, echo = TRUE}
X.s <- X/sd(X)        # standardize
X.s.T <- X.s[D==1]    # matching variable observations for treated observations
X.s.C <- X.s[D==0]    # matching variable observations for control observations
Y.Control <- Y[D==0]  # outcome variable observations for control observations

dist <- matrix(NA, nrow=length(X.s.T), ncol=length(X.s.C)) 
Ybar.C <- minimalDistance <- rep(NA, length(X.s.T))

for (i in 1:length(X.s.T)){
   # for each matching variable's treated observation, get distances to all control obs.
  dist[i,] <- abs(X.s.T[i] - X.s.C) 
  minimalDistance[i] <- min(dist[i,]) # find the smallest distance
  if (minimalDistance[i] < 1){ # only match if inside caliper
    matches.i <- which(dist[i,]==minimalDistance[i]) # match all that share closest distance (ties)
    # i.e., if there are several matches, each only gets proportional weight
    Ybar.C[i] <- mean(Y.Control[matches.i]) 
  }
}

Y.T <- Y[D==1] # outcome variable observations for treated observations
Y.T <- Y.T[(minimalDistance < 1)] # treated observations that have a match
Ybar.T <- mean(Y.T) # their mean

# mean outcome for the (weighted) matched control observations
Ybar.MatchedControlsLoop <- mean(Ybar.C, na.rm = T) 
Ybar.T - Ybar.MatchedControlsLoop
...
```{r, echo=F, cache=F, out.extra = 'style="display:block; margin-right:auto; margin-left:auto;"', fig.height=3.5, out.width='55%', dpi=190, fig.cap=paste0(fig_counter(), ': A DGP with treatment propensity'), fig.align='center',include=FALSE}
coord_dag <- list(
  x = c(a = 2, b = 5, c = 8, d = 2,e=2,f=8 ),
  y = c(a = 3, b = 3, c = 3, d = 2,e=1,f=1 )
)
dag_object <- ggdag::dagify(d ~ a,
                            d ~ b,
                            d ~ c,
...
```{r, echo=F, cache=F, fig.cap=paste0(fig_counter(), ': A DGP with treatment propensity')}
m <- DiagrammeR::grViz("dot_files/CD_pg15.dot", height = 250, width = "100%")
widgetframe::frameableWidget(m)
```
...
.blockquote[
### Example: Defaulting on credit card debt
```{r, echo=F, cache=F, out.extra = 'style="display:block; margin-right:auto; margin-left:auto;"', fig.height=3.5, out.width='70%', dpi=190, fig.cap=paste0(fig_counter(), ': UCI Credit Card data'), fig.align='left'}
ggplot(sub, aes(x = BillApril, y = yjit, shape = LateApril)) + 
  ...
  theme(text         = element_text(size = 13),
        ...
        axis.ticks.y = element_blank()# ,legend.position.inside = c(.7, .8),legend.background = element_rect(color = 'black')
        )
```
]]
...
- Lastly, we look for a match in the control group with a similar predicted probability. Once again we find a great match in row 27,281 with a predicted probability of .116.
]]
...
- The process of selecting matches means that control observations are picked to either be in or out of the matched control sample.
...
- Kernel functions are functions that, given a difference, return a weight. The highest weight is at a value of 0 (no difference) and the weights decline as the absolute difference becomes greater
...
<!--
- Kernel functions are functions that given a difference, return a weight. The highest value is at 0 (no difference), and then the valued uniform weights declines as the value move away from 0 in either direction
-->
...
<div class="thecontainer">
  <div id="observablehq-theplot-ba5c934a"></div>
  <div class="right-aligned">
    <ul style="list-style-type:none;">
      <li><div id="observablehq-viewof-therkernel-ba5c934a"></div></li>
    </ul>
  </div>
</div>
...
<script type="module">
import {Runtime, Inspector} from "https://cdn.jsdelivr.net/npm/@observablehq/runtime@5/dist/runtime.js";
import define from "https://api.observablehq.com/d/7d15f70ba784b0bd.js?v=4";
new Runtime().module(define, name => {
  if (name === "theplot") return new Inspector(document.querySelector("#observablehq-theplot-ba5c934a"));
  if (name === "viewof therkernel") return new Inspector(document.querySelector("#observablehq-viewof-therkernel-ba5c934a"));
...
```{r , echo=F, cache=F, out.extra = 'style="display:block; margin-right:auto; margin-left:auto;"', fig.height=3.5, out.width='65%', dpi=190, fig.cap=paste0(fig_counter(), ': Epanechnikov kernel'), fig.align='center'}
...
Two differences are calculated:
 ...
 - These differences are standardized (why?) to $\frac{21}{59.994}=0.00035$ and $\frac{34}{59.994}=0.00056$, where $59.994$ is the standard deviation of $Bill April$. 
]]
...
- $\frac{3}{4}(1-0.00035^2)=0.7499999$
- $\frac{3}{4}(1-0.00056^2)=0.7499998$
...
$$\frac{0.7499999 \cdot 0 + 7499998 \cdot 1}{0.7499999 + 0.7499998}=0.4999999$$ 

The treatment effect ist $0-0.4999999=-0.4999999$ 
]
...
- Some matching approaches end up using calipers/bandwidths naturally:
    ...
    Typical kernel-based matching approach will place a weight of 0 on any match that is far away that the kernel function that is chosen sets it to 0.
...
- However a wider bandwidth allows for more bad matches, which makes the match quality worse: the idea that the back doors are getting closed gets less plausible!
...
- **Expectation:** Black legislators respond more often to out-of-district emails from black people. 

- Matching comes in when we compare the in/out-of-district gap between Black and non-Black legislators. Those groups tend to be elected in very different kinds of places. Back doors abound. Matching can be used to make for a better comparison group.
]]
...
- Compute the distance: for a given treated observation A and a given control observation B, the Mahalanobis distance $d$ between two vectors $x_1$ and $x_2$ belonging to observations A and B is $$\\d(x_1,x_2)= \sqrt[]{(x_1-x_2)'S^{-1}(x_1-x_2)}.$$
...
<div class="thecontainer">
  <div id="observablehq-theplot-a5f86017"></div>
  <div id="observablehq-styles-a5f86017"></div>
  <div class="right-aligned">
    <ul style="list-style-type:none;">
      <li><div id="observablehq-viewof-ED-a5f86017"></div></li>
      <li><div id="observablehq-viewof-caliper-a5f86017"></div></li>
      <li><div id="observablehq-viewof-correlationFactor-a5f86017""></div></li>
    </ul>
    <ul>
      <li>The dashed circle shows a perimeter of the specified <b>Euclidean distance</b> (ED) around the selected observation. These are potential ED matches for the selected observation.</li>
      <li>Observations (of the other group) within the specified <b>Mahalanobis distance</b> (MHD) to the selected observation are highlighted with a cyan outline. These are potential MHD matches for the selected observation.</li>
      <li> \(\text{SD}(V1) = \text{SD}(V2) = \sqrt{120}\approx 10.95\). So \(\text{MHD} = 1\) is equivalent to \(\text{ED} = 10.95\) for \(\text{Corr}(V1,V2)=0\) (why?).</li>
      <li> Watch what happens when we introduce correlation!</li>
    </ul>
  </div>
</div>
...
<script type="module">
import {Runtime, Inspector} from "https://cdn.jsdelivr.net/npm/@observablehq/runtime@5/dist/runtime.js";
import define from "https://api.observablehq.com/d/4da2bc277fec4645@876.js?v=4";
new Runtime().module(define, name => {
  if (name === "theplot") return new Inspector(document.querySelector("#observablehq-theplot-a5f86017"));
  if (name === "viewof ED") return new Inspector(document.querySelector("#observablehq-viewof-ED-a5f86017"));
  if (name === "viewof caliper") return new Inspector(document.querySelector("#observablehq-viewof-caliper-a5f86017"));
  if (name === "viewof correlationFactor") return new Inspector(document.querySelector("#observablehq-viewof-correlationFactor-a5f86017"));
  return ["covXY","covarianceMatrix","sigmasqrt","det","inverseMatrix","thepoints","MahalanobisDistance","middleBluePoint","xc","yc"].includes(name);
});
</script>
...
Entropy balancing enforces restrictions in terms of moment conditions (means, variances, and so on) on the distance between treatment and control for matching variables:
...
- Treat the treated and control groups separately and look for the range of values of the propensity score where its density is zero (or close to zero, below some cutoff value). Then, drop all observations in the other group that have propensity scores in the range where the propensity score is too small in the current group.
...
- Worse matching of values leads to **bad balances** which in turn leads to **bias!**
...
<li> To get standard errors, use bootstrap standard errors, or suitable heteroskedasticity-robust errors</li>
</ol>
...
- The ATE can then be obtained as the average treatment affect for treated and untreated $$\text{ATE} = (a \times \text{ATT} ) + (1-a) \times \text{ATUT},$$ where $a$ is the proportion of treated individuals.
