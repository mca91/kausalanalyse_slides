---
title: "<a href='https://github.com/mca91/kausalanalyse_slides/raw/SoSe26/CausalPaths/CausalPaths.pdf'>Causal Paths</a>"
...
```{r , echo=F, cache=F, out.extra = 'style="display:block; margin-right:auto; margin-left:auto;"', fig.height=4, out.width='65%', dpi=190, fig.cap=paste0(fig_counter(), ': Causal paths'), fig.align='center', include=FALSE}
coord_dag <- list(
...
```
<br>
```{r, echo=F, cache=F, fig.cap=paste0(fig_counter(), ': Causal paths')}
m <- DiagrammeR::grViz("dot_files/Causal_Paths.dot", height = 350, width = "100%")
...
3. Repeat step 2 until you either... 
    ...
    come to a variable that has been already visited (_a loop_  &mdash; _that is not a path_)
...
.pull-left[
```{r, out.width='90%', out.extra = 'style="display:block; margin-right:auto; margin-left:auto;"', fig.height=5.5, dpi=190, echo=FALSE, include=FALSE}
coord_dag <- list(
...
```{r, echo=F, cache=F, fig.cap=paste0(fig_counter(), ': Causal paths')}
m <- DiagrammeR::grViz("dot_files/Causal_Paths_pg7.dot", height = 300, width = "100%")
widgetframe::frameableWidget(m)
```
...
```{r,  echo=FALSE, out.width='70%', out.extra='style="display:block; margin-right:auto; margin-left:auto;"', fig.height=3.5, dpi=190, fig.cap=paste0(fig_counter(), ': Causal paths'),include=FALSE}
coord_dag <- list(
...
```{r, echo=F, cache=F, fig.cap=paste0(fig_counter(), ': Causal paths')}
m <- DiagrammeR::grViz("dot_files/Causal_Paths_pg8.dot", height = 300, width = "100%")
widgetframe::frameableWidget(m)
```
...
- If we are looking for alternate explanations of why treatment and outcome might be related, the collider shuts down that alternate explanation.
 - If the path were *Treatment* ← *C* → *Outcome* without a collider, then one reason why *Treatment* and *Outcome* vary together is because *C* causes them both. But with a collider, *Treatment* ←  *A* → *B* ←   *C*  → Outcome ,  *C* can affect *Outcome*, and *C* can affect *B*, but because *B* doesn’t affect *Treatment*, *C* can no longer induce a relationship between *Treatment* and *Outcome*. *B* saved us.
...
Consider *Income* and *Drugs*. We list all paths between *Drugs* and *Income*
...
- Our diagram makes the unrealistic claim that *Income* and *Drugs* only relate to each other through *Wine*. 
  ...
  We can test if *Income* and *Drugs* are related after controlling for *Wine* using our data. If they are, our model is incomplete.
...
- Failing a placebo test indicates that the model is incorrect and incomplete.
