---
title: "<a href='https://github.com/mca91/kausalanalyse_slides/raw/SoSe26/TreatmentEffects/TreatmentEffects.pdf'>Treatment effects</a>"
...
**How to deal with heterogeneous treatment effects?**

- We can try to **estimate the heterogeneous treatment effects** &mdash; often, it is difficult &mdash; so we will instead focus on other things we can do with the concept of heterogeneous treatment effects, like answering the question of what is being identified if the effects are so heterogeneous.
...
- We might end up with the **average treatment effect on the treated (ATT)** or **the average treatment effect on the untreated (ATUT)**, which averages the treatment effects among those who actually got treated (or not).
...
.vcenter[
.blockquote[
### ATE and source of treatment variation
...
- However, the treatment effect we get is often entirely determined by the source of treatment variation we use. 
...
```{r, echo=F, cache=F, out.extra = 'style="display:block; margin-right:auto; margin-left:auto;"', fig.height=3.5, out.width='70%', dpi=190, fig.cap=paste0(': Causal diagram of Effect of traffic school on future driving performance '), fig.align='center',include=FALSE}
coord_dag <- list(
  x = c(a = 1, b = 3, c = 2, d= 1 ),
  y = c(a = 1, b = 1, c = 2, d=2 )
)
