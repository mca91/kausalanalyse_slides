...
- The data from the pre-treatment period is used to adjust for differences between the treatment and control groups. We then checked how they differ after treatment goes into effect. 
...
<br>
- Unlike with DiD, the pre-treatment difference adjustments are not done with regression, but rather using matching. 
...
- Matching on the outcome variable seems problematic: using the outcome itself to close back doors is cheating.

- Synthetic control only matches on pre-treatment outcomes, not the post-treatment outcomes where we actually estimate the effect
