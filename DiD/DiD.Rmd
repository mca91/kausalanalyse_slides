...
- Differences-in-differences (DiD) considers a second group that is *never* treated (another back door) to introduce within variation to compare too
...
<br>
.font80[(Death rates are deaths per 10,000 of the 1851 population.)]
]]
...
- DiD cannot work without parallel trends: 
  ...
  If the gap between the two groups would have changed from the pre-period to the post-period without treatmetn, then this non-treatment-related change will get mixed up with the treatment-related change.
...
<br>
.blockquote[
### Example: Plausible vs. implauslible prior trends
```{r, echo=F, cache=T, out.extra = 'style="display:block; margin-right:auto; margin-left:auto;"', fig.height=4, fig.width=9, out.width='80%', dpi=120, fig.cap=paste0(fig_counter(), ': A Graph Where the Prior Trends Test Looks Good for DID, and a Graph Where It Does not '), fig.align='center'}
set.seed(1000)
set.seed(1000)
suppressWarnings({
...
1. Ignoring all the data from the periods where treatment was actually applied

2. Using pre-treatment data, pick a few different periods and pretend that a treatment was applied at that time. 

3. Obtain DiD estimates at the pretended treatment dates. 

If a DID “effect” is found consistently at those pretended treatment dates, that gives a clue that something may be awry about the parallel trends assumption.
...
- By standard interaction-term interpretation, $\beta_3$ tells us how much bigger the $\text{TreatedGroup}$ effect is in the $\text{AfterTreatmen}$ than in the before-period

- Whichever the equation is written, this approach is called the *two-way fixed effects DiD estimator* since it has two sets of fixed effects, one for group and one for time period 
...
- Highly intuitive, straightforward to use with with software that implements fixed effects estimation
...
- Estimate
$$Y_k = \alpha_g + \alpha_t + \beta_{-T_1}\text{Treated}+\beta_{-(T_1-1)} \text{Treated} + \dots + \beta_1 \text{Treated} + \dots + \beta_{T_2} \text{Treated} + \epsilon,$$
 
  where the $\text{Treated}$ regressors are interactions of $\text{Treatment}$ and each of the time periods (for $T_1$ periods before the treatment period and $T_2$ periods afterwards).
...
.blockquote[
### Example: Effect of active choice on organ doner &mdash; ctd.
...
- For the treated group, $\beta_3$ gives us
