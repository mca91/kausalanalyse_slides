---
title: "<a href='https://github.com/mca91/kausalanalyse_slides/raw/SoSe26/RDD/RDD.pdf'>Regression discontinuity</a>"
...
---
layout: true
<a style="position: absolute;top:5px;left:10px;color:#004c93;" target="Overview"  href="https://kasa26s.netlify.app/">`r fontawesome::fa("home", height="15px")`</a>
...
- There are plenty of situations where treatment is assigned *discontinuously* : 
...
People close to the cutoff need to effectively be randomly assigned. We must rule out any obvious impediments to that randomness:
  ...
  1. People cannot manipulate the running variable (i.e., choose treatment or not) 
...
```{r, echo=F, cache=F, out.extra = 'style="display:block; margin-right:auto; margin-left:auto;"', fig.height=3, out.width='70%', dpi=190, fig.cap=paste0(fig_counter(), ': A causal diagram that regression discontinuity works for'), fig.align='center',include=FALSE}
coord_dag <- list(
  x = c(a = 1, b = 3, c = 5, d = 1,e=3 ),
...
<br>
```{r, echo=F, cache=F, fig.cap=paste0(fig_counter(), ': A causal diagram that regression discontinuity works for')}
m <- DiagrammeR::grViz("dot_files/Treatment_Outcome_pg7.dot", height = 250, width = "100%")
...
<br>
- The **running variable** has
 
 - ... a back door through $\text{Z}$.
 
 - ... a direct effect on $\text{Outcome}$.

- Controlling for $\text{Running variable}$ would close the back door $\text{Z}\rightarrow\text{Treatment}$

- We are however primarily interested in the effect of $\text{Treatment}$ which has a back door through $\text{Running Variable}$
...
```{r, echo=F, cache=T, out.extra='style="display:block; margin-right:auto; margin-left:auto; margin-top:30px;"', fig.height=5, fig.width=12, out.width='100%', out.height='95%', dpi=190, fig.cap=paste0(fig_counter(), ': Regression discontinuity step-by-step'), fig.align='center'}
suppressWarnings({
set.seed(1000)
...
- In these cases we have a fuzzy regression discontinuity design (FRDD), as opposed to a 'sharp' RDD where the probability of treatment jumps from 0% to 100%
...
<br>
```{r, echo=F, cache=F, out.extra = 'style="display:block; margin-right:auto; margin-left:auto;"', fig.height=4, fig.width=12, out.width='90%', out.height='95%', dpi=190, fig.cap=paste0(fig_counter(), ': Sharp vs Fuzzy Discontinuity'), fig.align='center'}
suppressWarnings({
...
```{r, echo=F, cache=F, out.extra = 'style="display:block; margin-right:auto; margin-left:auto;"', fig.height=4.5, out.width='55%', dpi=190, fig.cap=paste0(fig_counter(), ': Fuzzy discontinuity in pension rate'), fig.align='center'}
suppressWarnings({
...
- Roughly, Battistin et al. divide the estimate by the observed 30% jump to see how big the change would have been *if everyone got treated*.
...
- This is a **local average treatment effect** (LATE):
  ...
  A LATE is a weighted average treatment effect for those just on the margin of being given treatment.
...
- $\beta_2$ then is our treatment effect.
...
- Controls may be necessary for FRDDs, where there are determinants of treatment other than the cutoff: there may then be back doors to be worried about. 
...
.vcenter[
```{r, echo=F, cache=F, out.extra = 'style="display:block; margin-right:auto; margin-left:auto;"', fig.height=4, out.width='60%', dpi=190, fig.cap=paste0(fig_counter(), ': Sharp regression discontinuity estimated with linear regression with an interaction'), fig.align='center'}
suppressWarnings({
set.seed(1000)
...
- The problem is especially bad here because fitted shapes tend to be at their most wrong *at the edges* of the available data (can you give an example?) 
...
E.g., second order polynomials could be used.
...
```{r , echo=F, cache=F, out.extra = 'style="display:block; margin-right:auto; margin-left:auto;"', fig.height=4, out.width='60%',dpi=190, fig.cap=paste0(fig_counter(), ': Regression Discontinuity Estimated with Linear Regression with an Interaction, both Without and With a Bandwidth Restriction'), fig.align='center'}
suppressWarnings({
set.seed(1000)
tib <- tibble(X = runif(200)) %>%
  mutate(Y = rnorm(200)*.05 + case_when(
...
---
## Regression discontinuity with OLS
#### A linear shape may not be flexible enough

<div class="thecontainer">
  <div id="observablehq-svg-ec9bc373"></div>
  <div id="observablehq-styles-ec9bc373"></div>
  <div class="right-aligned">
    <ul style="list-style-type:none;">
      <li><div id="observablehq-viewof-showTruth-3d220834"></div></li>
      <li><div id="observablehq-viewof-est-ec9bc373"></div></li>
      <li><div id="observablehq-viewof-lowerPoly-ec9bc373"></div></li>
      <li><div id="observablehq-viewof-upperPoly-ec9bc373"></div></li>
      <li><div id="observablehq-viewof-bw_left-ec9bc373"></div></li>
      <li><div id="observablehq-viewof-bw_right-ec9bc373"></div></li>
    </ul>
  </div>
</div>
...
import define from "https://api.observablehq.com/d/74f688f1119258d6@224.js?v=4";
new Runtime().module(define, name => {
  if (name === "svg") return new Inspector(document.querySelector("#observablehq-svg-ec9bc373"));
  if (name === "viewof est") return new Inspector(document.querySelector("#observablehq-viewof-est-ec9bc373"));
  if (name === "viewof showTruth") return new Inspector(document.querySelector("#observablehq-viewof-showTruth-3d220834"));
  if (name === "viewof lowerPoly") return new Inspector(document.querySelector("#observablehq-viewof-lowerPoly-ec9bc373"));
  if (name === "viewof upperPoly") return new Inspector(document.querySelector("#observablehq-viewof-upperPoly-ec9bc373"));
  if (name === "viewof bw_left") return new Inspector(document.querySelector("#observablehq-viewof-bw_left-ec9bc373"));
  if (name === "viewof bw_right") return new Inspector(document.querySelector("#observablehq-viewof-bw_right-ec9bc373"));
  if (name === "styles") return new Inspector(document.querySelector("#observablehq-styles-ec9bc373"));
});
</script>
...
```{r, echo=F, cache=F, out.extra = 'style="display:block; margin-right:auto; margin-left:auto; margin-top:40px;"', fig.height=5.25, fig.width=10, out.width='80%', dpi=190, fig.cap=paste0(fig_counter(), ': Regression Discontinuity with Different Polynomials'), fig.align='center'}
suppressWarnings({
set.seed(1000)
tib <- tibble(X = runif(200)) %>%
  mutate(Y = rnorm(200)*.05 + case_when(
...
- For each value of $X$, we estimate the corresponding value of $Y$ by running its own regression
- This 'dedicated' regression fits a specific shape
...
.vcenter[
```{r, echo=F, cache=F, out.extra = 'style="display:block; margin-right:auto; margin-left:auto;"', fig.height=4, out.width='60%', dpi=190, fig.cap=paste0(fig_counter(), ': Triangular kernel function'), fig.align='center'}
suppressWarnings({
...
<div class="thecontainer">
  <div id="observablehq-theplot-78019354"></div>
  <div class="right-aligned">
    <ul style="list-style-type:none;">
      <li> <div id="observablehq-viewof-estimatorLabels-78019354"></div></li>
      <li> <div id="observablehq-viewof-thekernel-78019354"></div></li>
      <li> <div id="observablehq-viewof-bw_type-78019354"></div></li>
      <li> <div id="observablehq-viewof-bandwidth-78019354"></div></li>
      <li> <div id="observablehq-viewof-thevalue-78019354"></div></li>
    </ul>
  </div>
</div>
...
<script type="module">
import {Runtime, Inspector} from "https://cdn.jsdelivr.net/npm/@observablehq/runtime@5/dist/runtime.js";
import define from "https://api.observablehq.com/d/6ad2c88d5f52fa8d@1361.js?v=4";
new Runtime().module(define, name => {
  if (name === "viewof estimatorLabels") return new Inspector(document.querySelector("#observablehq-viewof-estimatorLabels-78019354"));
  if (name === "viewof thekernel") return new Inspector(document.querySelector("#observablehq-viewof-thekernel-78019354"));
  if (name === "viewof bw_type") return new Inspector(document.querySelector("#observablehq-viewof-bw_type-78019354"));
  if (name === "viewof bandwidth") return new Inspector(document.querySelector("#observablehq-viewof-bandwidth-78019354"));
  if (name === "viewof thevalue") return new Inspector(document.querySelector("#observablehq-viewof-thevalue-78019354"));
  if (name === "theplot") return new Inspector(document.querySelector("#observablehq-theplot-78019354"));
  return ["estimator","mloess"].includes(name);
});
</script>
...
---
## Discontinuity designs and local regression
#### Local linear Regression with tricube kernel


<div class="thecontainer">
 <div id="observablehq-svg-15c7fb61"></div>
 <div id="observablehq-styles-15c7fb61"></div>
  <div class="right-aligned">
    <ul style="list-style-type:none;">
      <li><div id="observablehq-viewof-bandwidth-15c7fb61"></div></li>
    </ul>
  </div>
</div>
...
<script type="module">
import {Runtime, Inspector} from "https://cdn.jsdelivr.net/npm/@observablehq/runtime@5/dist/runtime.js";
import define from "https://api.observablehq.com/d/701ab8e4d42fd456.js?v=4";
new Runtime().module(define, name => {
  if (name === "svg") return new Inspector(document.querySelector("#observablehq-svg-15c7fb61"));
  if (name === "viewof bandwidth") return new Inspector(document.querySelector("#observablehq-viewof-bandwidth-15c7fb61"));
  if (name === "styles") return new Inspector(document.querySelector("#observablehq-styles-15c7fb61"));
});
</script>
...
.blockquote[
### Example: Manacorda et al. (2011)
```{r, echo=F, cache=F, out.extra = 'style="display:block; margin-right:auto; margin-left:auto;"', fig.height=4, out.width='60%', dpi=190, fig.cap=paste0(fig_counter(), ': Government support by income with policy cutoff'), fig.align='center'}
...
binned <- gt %>%
    mutate(Inc_Bins = cut(Income_Centered,
           breaks = (-15:15)*(.02/15))) %>%
    group_by(Inc_Bins) %>%
    dplyr::summarize(Support = mean(Support),
...
.blockquote[
```{r, echo=F, cache=T, warning=F, out.extra='style="display:block; margin-right:auto; margin-left:auto;"'}
gt <- causaldata::gov_transfers
...
msummary(
  models = list('Quadratic' = m, 'Linear with kernel weight' = mw), 
  output = "huxtable",
  estimate = "estimate",
  stars = c('*' = .1, '**' = .05, '***' = .01), 
  vcov = 'robust', gof_omit = "") %>%
  huxtable::set_text_color(row = 6:7, col = 1:ncol(.), value = 'red') %>%
  huxtable::set_font_size(13) %>%
  huxtable::set_bottom_padding(4) %>% 
  huxtable::set_top_padding(4) %>% 
  huxtable::set_width(.75)
```
]
...
```{r, echo=F, cache=F, out.extra = 'style="display:block; margin-right:auto; margin-left:auto;"', fig.height=3.5, out.width='60%', dpi=190, fig.cap=paste0(': A causal diagram that fuzzy regression discontinuity works for'), fig.align='center',include=FALSE}
coord_dag <- list(
  x = c(a = 1, b = 3, c = 5, d = 1,e=3 ),
  y = c(a = 1, b = 1, c = 1, d = 2,e=2 )
)
...
<br>
<br>
```{r, echo=F, cache=F, fig.cap=paste0(fig_counter(), ': A causal diagram that fuzzy regression discontinuity works for')}
m <- DiagrammeR::grViz("dot_files/Treatment_Outcome_pg33.dot", height = 300, width = "100%")
widgetframe::frameableWidget(m)
```
...
&rarr; The effect of being *above* the cutoff on the outcome is scaled to account for the fact that being above the cutoff only leads to a partial increase in treatment rates.
...
- The “treatment” of being eligible for mortgage subsidies would only apply to some people born at the right time: 
  ...
  Treatment rates jump from 0% to some value below 100% (this is fuzzy).

- Veteran status at this margin increases home ownership rates by 17%

- The RDD effect is the *ratio of the jumps* in eligible veteran rates and home ownership rates at the cutoff.
...
.blockquote[
### Example: Effect of mortgage subsidies on home ownership (Fetter 2013)
```{r, echo=F, cache=F, out.extra = 'style="display:block; margin-right:auto; margin-left:auto;"', fig.height=4, fig.width=8, out.width='70%', dpi=190, fig.cap=paste0(fig_counter(), ': Eligibility for mortgage subsidies for being a Korean war veteran and home ownership from Fetter (2013)'), fig.align='center'}
vet <- read_csv('fetter_mortgages.csv')

vet <- vet %>%
  filter(abs(qob_minus_kw) < 12) %>%
  group_by(qob_minus_kw) %>%
  dplyr::summarize(mean_treat = mean(vet_wwko), 
...
Anything we would normally use as a control variable should not be affected by treatment.
...
Keep in mind that, since we can run placebo tests on a long list of potential placebo outcomes, it is likely that we find a few nonzero effects just by random chance. 
  ...
  &rarr; If one tests a long list of variables and finds a few differences, that is not a fatal problem with the design.
...
```{r, echo=F, cache=F, out.extra = 'style="display:block; margin-right:auto; margin-left:auto;"', fig.height=4, fig.width=8, out.width='70%', dpi=190, fig.cap=paste0(fig_counter(), ': Performing regression discontinuity with controls as outcomes for a placebo test'), fig.align='center'}
...
In the case of indirect control we do have a test to check whether manipulation seems to be occurring at the cutoff.
...
- Also do a graphical inspection of the density
...
.blockquote[
### Example: Manacorda et al. (2011)
```{r, echo=F, warning=F, cache=F, out.extra = 'style="display:block; margin-right:auto; margin-left:auto;margin-top:40px;"', fig.height=4, fig.width=8, out.width='75%', dpi=190, fig.cap=paste0(fig_counter(), ': Distribution of the binned running variable'), fig.align='center'}
...
```{r, echo=F, cache=F, out.extra = 'style="display:block; margin-right:auto; margin-left:auto;"', fig.height=3, out.width='55%', dpi=190, fig.cap=paste0(fig_counter(), ': Regression kink on simulated data'), fig.align='center'}
set.seed(1001)
...
- Bana et al. look at the impact of paid family leave in California
...
- After that, additional regular earnings do not increase the family leave payment
...
```{r, echo=F, cache=F, out.extra = 'style="display:block; margin-right:auto; margin-left:auto;"', fig.height=4, out.width='60%', dpi=190, fig.cap=paste0(fig_counter(), ': Paid family leave benefits and pre-leave Earnings'), fig.align='center'}
...
```{r, echo=F, cache=F, out.extra = 'style="display:block; margin-right:auto; margin-left:auto;margin-top:40px;"', fig.height=4, fig.width=10, out.width='90%', dpi=190, fig.cap=paste0(fig_counter(), ': Log leave duration or proportion claiming leave again and pre-leave earnings'), fig.align='center'}
...
5258.428753180662, 2.333957485402334') %>%
  ...
  p1 <- ggplot(tb1, aes(x = x, y = y, group = x > 0)) + 
    ...
    geom_vline(aes(xintercept = 0), linetype = 'dashed') + 
    labs(y = 'Log duration of family leave',
         x = 'Regular earnings - cutoff') + 
    theme_pubr() +
    theme(text = element_text( size = 14)) +
...
') %>%
  ...
  p2 <- ggplot(tb2, aes(x = x, y = y, group = x > 0)) + 
    ...
    geom_smooth(se = FALSE, method = 'lm', color = 'black') +
    geom_vline(aes(xintercept = 0), linetype = 'dashed') + 
    labs(y = 'Proportion claiming leave again',
         x = 'Regular earnings - cutoff') + 
    theme_pubr() +
    theme(text = element_text( size = 14)) +
...
---
# `r stairs` When running variables misbehave
<br>
...
Granularity is when a variable is measured at a coarse level.

Too coarse a variable makes it difficult to justify that the treatment is the only thing separating observations below and above the cutoff.

</br>
**Example: Annual income**

'$40,231.36', '$40,231', '$40-50,000', and 'less than $100,000' are measurements of income in decreasing order of granularity.

We would like to find a running variable that is granular enough. So when worried about granularity, it is better to pick an estimator that will account for that granularity.
...
Non-random heaping is when the running variable takes certain values more often than others.

<br>

**Example: quantitative surveys** 

  Free response surveys with quantitative questions are mostly answered with a whole number ('How old are you?'). Some answers will be more precise ('36 years vs. 36 years, eight months, and two days'). 

<br>

**Donut hole regression discontinuity** drops observations just around the cutoff so as to clear out heaps near the cutoff.
  
---
# `r stairs` Dealing with bandwidths
<br>
...
- **Pick a bunch of bandwidths** 
  ...
  Sensitivity check: Pick the biggest bandwidth that seems sensible for the research design and then shrink it. Estimate the regression discontinuity model each time and compare.
...
**Data-based bandwidth selection**
 ...
 - **Cross-validation** 
  ...
  Compare estimates of out-of-sample predictive power: 
  
  1. Randomly split the data into chunks 
  
  2. For each chunk, obtain predictions from a model estimated using all the other chunks. Compute a measure of the overall prediction performance 
  
  3. Repeat that whole process for each potential bandwidth and see which one does the best
 
 - **Optimal bandwidth rules** 
 
  This approach aims to get the best prediction on either side right at the cutoff.
