...
# Motivation 
.vcenter[
- Modelling the DGP, listing all the paths, finding a set of variables that close all the back doors, and measuring and controlling for all of those variables is a lot of work by itself.

- Controlling for everything is difficult, especially in social science where  many things might matter and you might run into a variable you have to control for but you cannot.
- There are so many variables that might be on back doors, there are high chances to forget some of them in your causal diagram.
- An alternate approach to identifying the answer to a research question is to, instead of actively closing back doors, find ways of isolating just front doors. 
]
...
- If the treatment was assigned randomly, then for everyone in the experiment, variation in all the variables on all the back doors should be unrelated to whether they got the treatment or not. Hence, all the back doors will be closed.
...
- We are interested in figuring out whether charter schools improve students’ test scores than traditional public schools.
- Many variable influence people's decision to attend Charter school,for example, background, personality, location, etc.
...
Natural experiments are a real-world settings in which randomisation of treatment and control conditions has been already done by nature or by other factors that our outside our control
...
Anything which is random in context of our DGP can be considered as a source of exogenous variation.
...
- Natural randomness can have *pure back doors to the outcome* but pure randomisation does not.
...
-  The observations from natural experiments are more realistic than randomised controlled experiments.
...
- The front door method works when your causal diagram looks something like the following figure, when there is a bad path that cannot be closed, such as if W in that diagram cannot be measured.
