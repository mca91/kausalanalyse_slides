---
title: "<a href='https://github.com/mca91/kausalanalyse_slides/raw/WiSe25/CausalDiagrams/CausalDiagrams.pdf'>Causal Diagrams</a>"
...
Obvious ones:
 - A light switch being set to on causing the light to be on
...
- $X$ causes $Y$ ("Many studies suggest that eating chocolate causes weight gain")
- $X$ affects $Y$ ("Various factors affect the success of a business")
- The effect of $X$ on $Y$ ("The effect of social media on mental health is thought to be significant")
- $X$ influences $Y$ ("The effect of diet influences overall health")
...
A causal diagram is a graphical representation of a data generating process (DGP). Variables in the DGP are represented by nodes, and the causal relationships in the DGP are depicted by arrows, with each arrow pointing from the cause variable to the effect variable.
]
...
```{r, echo=F, cache=F, out.extra = 'style="color:black; display:block; margin-right:auto; margin-left:auto; margin-top:15px"', fig.height=4.5, out.width='60%', dpi=190, fig.cap=paste0(fig_counter(), ': A simple causal diagram'), include = F}
coord_dag <- list(
  x = c(Y = 3, X = 1),
  y = c(Y = 1, X = 1)
)
...
```{r, echo=F, cache=F, fig.cap=paste0(fig_counter(), ': A simple causal diagram')}
m <- DiagrammeR::grViz("dot_files/CoinFlip.dot", height = 80, width = "100%")
...
- Other events that might affect the outcomes are ignored.
- Outcomes of outcomes are not taken into consideration.
- All (non-trivial) variables relevant to the DGP need to be included, even if we cannot measure or see them.
...
```{r, echo=F, cache=F, out.extra = 'style="color:black; display:block; margin-right:auto; margin-left:auto; margin-top:15px"', fig.height=4.5, out.width='52%', dpi=190, fig.cap=paste0(fig_counter(), ': Unobserved variable'), include = F}
coord_dag <- list(
  x = c(X = 1, U = 3, Y = 5),
  y = c(X = 1, U = 2, Y = 1)
)
...
```
<br>
```{r, echo=F, cache=F, fig.cap=paste0(fig_counter(), ': Unobserved variable')}
m <- DiagrammeR::grViz("dot_files/IceCream_Shorts.dot", height = 200, width = "100%")
widgetframe::frameableWidget(m)
```
...
These are arrows that are <u>not</u> there. These are just as important parts of a DAG!
...
```{r, echo=F, cache=F, out.extra = 'style="color:black; display:block; margin-right:auto; margin-left:auto; margin-top:5%"', fig.width=10, fig.height=4, dpi=190, fig.cap=paste0(fig_counter(), ': Police presence and crime rate'),include=FALSE}
coord_dag <- list(
...
<br><br>
```{r, echo=F, cache=F, fig.cap=paste0(fig_counter(), ': Police presence and crime rate')}
m <- DiagrammeR::grViz("dot_files/Police_Crime.dot", height = 350, width = "100%")
widgetframe::frameableWidget(m)
```
...
We may identify parts of the diagram that indicate how police per capita causes crime
...
```{r, echo=F, cache=F, out.extra = 'style="color:black; display:block; margin-right:auto; margin-left:auto; margin-top:1%"', fig.height=5, out.width='55%', dpi=190, fig.cap=paste0(fig_counter(), ': Z moderates X'),include=FALSE}
coord_dag <- list(
  x = c(X = 1, Y = 3, Z = 5),
  y = c(X = 2, Y = 1, Z = 2)
) 
...
```
<br>
```{r, echo=F, cache=F, fig.cap=paste0(fig_counter(), ': Z moderates X')}
m <- DiagrammeR::grViz("dot_files/ZModeratesX.dot", height = 250, width = "100%")
widgetframe::frameableWidget(m)
```
---
## Causal diagrams: Moderators
